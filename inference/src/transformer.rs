...
use crate::ops::{add, matmul_f32, matmul_q4k, rms_norm, rope, silu, softmax};
use crate::tensor::{BlockQ4K, Tensor, TensorData, QK_K};
use alloc::vec::Vec;
...
impl Transformer {
    ...
    fn embedding_lookup(&self, tokens: &[u32]) -> Result<Vec<f32>, ModelError> {
        ...
        match &self.weights.embedding.weight.data {
                ...
                for &token_id in tokens {
                    ...
                    if offset + hidden_size > weight_data.len() {
                        return Err(ModelError::InvalidInput(
                            "Embedding weight out of bounds".into(),
                        ));
                    }
            ...
            TensorData::Q4K(_) => {
                ...
                return Err(ModelError::InvalidInput(
                    "Quantized embeddings not yet supported".into(),
                ));
            }
        }
    ...
    fn attention_layer(
        ...
        if qkv.len() != qkv_len {
            return Err(ModelError::InvalidInput(
                "QKV projection size mismatch".into(),
            ));
        }
        ...
        for i in 0..seq_len {
            for h in 0..num_heads {
                let q_head =
                    &q_rope[(i * num_heads + h) * head_dim..(i * num_heads + h + 1) * head_dim];
                for j in 0..total_seq_len {
                    let k_head =
                        &k_full[(j * num_heads + h) * head_dim..(j * num_heads + h + 1) * head_dim];
                    let mut score = 0.0;
                    for d in 0..head_dim {
        ...
        for i in 0..seq_len {
            for h in 0..num_heads {
                ...
                let out_head = &mut attn_out
                    [(i * num_heads + h) * head_dim..(i * num_heads + h + 1) * head_dim];

                for j in 0..total_seq_len {
                    let score = scores[score_start + j];
                    let v_head =
                        &v_full[(j * num_heads + h) * head_dim..(j * num_heads + h + 1) * head_dim];
                    for d in 0..head_dim {
    ...
    fn qkv_projection(&self, x: &[f32], qkv_weight: &Tensor) -> Result<Vec<f32>, ModelError> {
        ...
        match &qkv_weight.data {
            ...
            TensorData::Q4K(_weight) => {
                ...
                return Err(ModelError::InvalidInput(
                    "Quantized QKV projection requires proper weight format or specialized matmul"
                        .into(),
                ));
            }
        }
    ...
    fn attention_output_projection(
        ...
        match &out_weight.data {
            ...
            TensorData::Q4K(weight) => {
                ...
                let out_transposed =
                    matmul_q4k(weight, &x_transposed, hidden_size, seq_len, hidden_size);
    ...
    fn ffn_layer(
        ...
        // 1. Gate projection
        let gate =
            self.ffn_projection(x, &layer.ffn_gate, hidden_size, intermediate_size, seq_len)?;
        ...
        // 5. Down projection
        let output = self.ffn_projection(
            &ffn_intermediate,
            &layer.ffn_down,
            intermediate_size,
            hidden_size,
    ...
    fn ffn_projection(
        ...
        match &weight.data {
            ...
            TensorData::Q4K(weight_data) => {
                ...
                let out_transposed =
                    matmul_q4k(weight_data, &x_transposed, in_dim, seq_len, out_dim);
