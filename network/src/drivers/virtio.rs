...
use crate::pci::{PciDevice, find_pci_device, VIRTIO_VENDOR_ID, VIRTIO_NET_DEVICE_ID};
use core::ptr;
use spin::Mutex;
extern crate alloc;
...
impl VirtioNet {
    ...
    pub fn init(&mut self) -> Result<(), NetError> {
        ...
        let driver_features = device_features & (VIRTIO_NET_F_MAC | VIRTIO_NET_F_STATUS | VIRTIO_F_VERSION_1);
        ...
        if (self.read_status() & VIRTIO_STATUS_FEATURES_OK) == 0 {
            return Err(NetError::VirtioError("Feature negotiation failed".to_string()));
        }
    ...
    fn init_queues(&mut self) -> Result<(), NetError> {
        ...
        let rx_memory = unsafe {
            ...
            if ptr.is_null() {
                return Err(NetError::QueueError("Failed to allocate RX queue memory".to_string()));
            }
            ptr
        };
        ...
        let tx_memory = unsafe {
            ...
            let ptr = alloc::alloc::alloc_zeroed(layout);
            if ptr.is_null() {
                return Err(NetError::QueueError("Failed to allocate TX queue memory".to_string()));
            }
            ptr
        };
    ...
    fn read_status(&self) -> u8 {
        unsafe {
            ptr::read_volatile((self.io_base + VIRTIO_PCI_STATUS as usize) as *const u8)
        }
    }
    ...
    fn write_status(&mut self, status: u8) {
        unsafe {
            ptr::write_volatile((self.io_base + VIRTIO_PCI_STATUS as usize) as *mut u8, status);
        }
    }
    ...
    fn read_device_features(&self) -> u64 {
        unsafe {
            let low = ptr::read_volatile((self.io_base + VIRTIO_PCI_DEVICE_FEATURES as usize) as *const u32) as u64;
            let high = ptr::read_volatile((self.io_base + VIRTIO_PCI_DEVICE_FEATURES as usize + 4) as *const u32) as u64;
            low | (high << 32)
        }
    }
    ...
    fn write_driver_features(&mut self, features: u64) {
        unsafe {
            ptr::write_volatile((self.io_base + VIRTIO_PCI_DRIVER_FEATURES as usize) as *mut u32, features as u32);
            ptr::write_volatile((self.io_base + VIRTIO_PCI_DRIVER_FEATURES as usize + 4) as *mut u32, (features >> 32) as u32);
        }
    }
...
impl NetworkDriver for VirtioNet {
    fn send(&mut self, packet: &[u8]) -> Result<(), NetError> {
        ...
        unsafe {
            ...
            if tx_buf.is_null() {
                return Err(NetError::QueueError("Failed to allocate TX buffer".to_string()));
            }
